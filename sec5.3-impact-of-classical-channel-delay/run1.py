...
def program(log_path, MEMO_FREQ, MEMO_EXPIRE, MEMO_EFFICIENCY, MEMO_FIDELITY, DETECTOR_EFFICIENCY, DETECTOR_COUNT_RATE, DETECTOR_RESOLUTION, ATTENUATION, QC_FREQ, SWAP_SUCC_PROB, SWAP_DEGRADATION):
    ...
    # update classical channel delay
    #     delay := propagation delay + 4us (packet processing time) + 8us (transmission delay)
    for cc in network_topo.cchannels:
        cc.delay = cc.distance / cc.light_speed + 4e6 + 8e6
...
if __name__ == "__main__":
    ...
    for C in [500]:
        for e in [0.75]:
            ...
            for freq in [200, 2000, 20000]:
                path = "low_delay/cavity_cooperativity_%d/efficiency_%.2f/memo_freq/%d" % (C, e, freq)
